...
/* 33-3. Vue.js 템플릿 ListToDetail 패턴 - EditView
  1. data에 id 선언, id를 받아오는지 확인 후 데이터를 불러올 수 있도록 작성
    created() {
      this.id = this.$route.params.id
      console.log(this.id)
      if (!window.daum) {
  ...
  2. 수정하기 폼 만들기 data에 customer: {} 오브젝트 추가
  3. input 추가 및 바로 바인딩 할 수 있도록 model 추가, 저장, 취소 버튼 추가
  4. 다음 우편번호 추가
  5. 취소 이벤트 추가
  6. 저장 이벤트 추가(31 저장 이벤트 참고), 저장 완료시 목록으로 가게끔 코드 추가, id 데이터가 맞는지 확인해보자
*/
</script>
