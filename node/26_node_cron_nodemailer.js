// 17. 정해진 시간마다 DB 데이터를 포함한 이메일 보내기
...
const task = cron.schedule(
  ...
  async () => {
    ...
    const h = [];
    h.push(`<table style="border:1px solid black; border-collapse:collapse;"`);// 메일로 보낼 때는 inline 방식으로 css style를 작성해 보내야함, class 적용 안 됨
    h.push(`<thead>`);
    h.push(`<tr>`);
...
// 서버 실행 
// postman에서 메일전송 시작 startTask get http://localhost:3000/task/start 추가 후 테스트
// postman에서 메일전송 종료 stopTask get http://localhost:3000/task/stop 추가 후 테스트